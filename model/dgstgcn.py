...
class DGBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, **kwargs):
        ...
        gcn_kwargs = {k[4:]: v for k, v in kwargs.items() if k[:4] == 'gcn_'}  # 'ratio': 0.125, 'ctr': 'T', 'ada': 'T'}
    ...
    def forward(self, x, A=None):  # # bs,C,T,V [4, 3, 100, 25]
        ...
        res = self.residual(x)
        x = self.tcn(self.gcn(x, A)) + res  #  [4, 64, 100, 25] <- [4, 64, 100, 25] 残差连接
...
class DGSTGCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.graph = Graph(**graph_cfg)  # K,V,V 直接是[8, 25, 25] dgstgcn种Figure 3中的 C/k,k是8，其实本质上就是ctrgc中通道的8,不过那个是细华后的8,然后A有三种，实际上就相当于有24种，这个直接就是8卡死了
        A = torch.tensor(self.graph.A, dtype=torch.float32, requires_grad=False)
        self.data_bn_type = data_bn_type
        self.kwargs = kwargs  # 'gcn_ratio':0.125'gcn_ctr':'T''gcn_ada':'T''tcn_ms_cfg': [(3, 1), (3, 2), (3, 3), (3, 4), ('max', 3), '1x1']
    ...
    def forward(self, x):  # N,M,T,V,C[2, 2, 100, 25, 3]
        N, M, T, V, C = x.size()
        ...
        if self.data_bn_type == 'MVC':
            ...
            x = self.data_bn(x.view(N * M, V * C, T))
        x = x.view(N, M, V, C, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, C, T, V)  # bs,C,T,V [4, 3, 100, 25]

        for i in range(self.num_stages):  # 10
            x = self.gcn[i](x)  # # bs,C,T,V [4, 3, 100, 25]  最终 [4, 256, 25, 25]

        x = x.reshape((N, M) + x.shape[1:])  # 
        return x  # [2, 2, 256, 25, 25]