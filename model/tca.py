...
class AFF(nn.Module):
    ...
    def forward(self, x, residual):  #   bs,C',T,V [4, 64, 64, 25]  Attentional Feature Fusion
        xa = x + residual
...
class TemporalConv(nn.Module):
    ...
    def forward(self, x):  # [4, 16, 64, 25]  已经进行Conv2d(64, 16, kernel_size=(1, 1), stride=(1, 1)) 
        x = self.conv(x)  #  [4, 16, 64, 25] <- Conv2d(16, 16, kernel_size=(5, 1), stride=(1, 1), padding=(2, 0))空洞卷积 [4, 16, 64, 25]
        x = self.bn(x)
        return x   # [4, 16, 64, 25]
...
class MultiScale_TemporalConv(nn.Module):
    ...
    def forward(self, x):  # bs,C',T,V[4, 64, 64, 25]
        # Input dim: (N,C,T,V)
        branch_outs = []
        ...
        out = torch.cat(branch_outs, dim=1)  # bs,C',T,V [4, 64, 64, 25] 就是MultiScale_TemporalConv步骤
        aff=self.af  # 比原来多的是加入了AFF模块，即通过注意力机制学习global和local权重再相乘，就是SE操作
        out=aff(out,0)  # [4, 64, 64, 25]
...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, adaptive=True, residual=True):
        ...
        for i in range(self.num_subset):  # 3
    ...
    def forward(self, x):  # # bs,C,T,V [4, 3, 64, 25]  类似于ctrgc三个并列
        y = None
        if self.adaptive:
            A = self.PA
        else:
        ...
        for i in range(self.num_subset):  # 3 
            z = self.convs[i](x, A[i], self.alpha)  #   # bs,C',T,V [4, 64, 64, 25] <- bs,c,T,V [4, 3, 64, 25], [25,25]
            y = z + y if y is not None else z  # y不空那么+,y空就是第一个ctrgc,那么就是本身
        y = self.bn(y)
        y += self.down(x)  # 简单的残差连接 [4, 64, 64, 25]<- [4, 64, 64, 25]  + conv2d(3,64) [4,3,64,25]
        y = self.relu(y)
...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x):  #   [4, 64, 64, 25]  完成ctrgcn步骤
...
class Model(nn.Module):
    ...
    def forward(self, x):  # bs,C,T,V,M [2, 3, 64, 25, 2]
        if len(x.shape) == 3:
            N, T, VC = x.shape
            x = x.view(N, T, self.num_point, -1).permute(0, 3, 1, 2).contiguous().unsqueeze(-1)
        N, C, T, V, M = x.size()

        x = x.permute(0, 4, 3, 1, 2).contiguous().view(N, M * V * C, T)  # N, MVC,T[2, 150, 64]
        x = self.data_bn(x)
        x = x.view(N, M, V, C, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, C, T, V)  # bs,C,T,V [4, 3, 64, 25]
        x = self.l1(x)  # # bs,C,T,V [4, 3, 64, 25]
        x = self.l2(x)
        x = self.l3(x)
        x = self.l4(x)
        x = self.l5(x)
