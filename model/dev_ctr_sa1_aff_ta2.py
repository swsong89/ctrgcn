...
class CTRGC(nn.Module):
    def __init__(self, in_channels, out_channels, rel_reduction=8, mid_reduction=1):
        ...
        # self.conv6 = nn.Conv2d(self.in_channels, self.rel_channels, kernel_size=1)  # 将维减少计算量
    ...
    def forward(self, x, A=None, alpha=1):  # x [bs*2, 3, step, 25] [4,3,64,25] A v,v [25,25]因为有三个ctrgc,所以将A分开了
        ...
        x3 = self.conv5(x)
        x3_view = x3.view(-1, T, V) #  bsC,T,V [4,8,64,25]
        x3_view_transpose = x3.view(-1, T, V).permute(0, 2, 1)
        attention = torch.bmm(x3_view, x3_view_transpose)
        attention_new = torch.max(attention, dim=-1, keepdim=True)[0].expand_as(attention) - attention
        attention = self.softmax(attention_new) # bsC,T,T
        # self.beta 权重 + x3 残差连接
        x3= torch.bmm(attention, x3_view).view(N, -1, T, V)*self.beta + x3  #  N,C,T,T  N,C,T,V  -> N,C,T,V
        x3 = self.conv3(x3)  # bs,C,T,V [4,64,64,25] <- conv2d(64, 8) [4,8,64,25]

        # 下面这部分是类似ctr通道拓扑建模
        x1 = self.conv1(x).view(-1, T, V).permute(0, 2, 1)  #  bsC,T,V -> bs,V,T
        x2 = self.conv2(x).view(-1, T, V) #  bsC,T,V
        x1 = self.softmax(torch.matmul(x1, x2)).view(N, -1, V, V) # bs, C,V,V
